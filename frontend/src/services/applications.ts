...
export async function getPublicMetrics(): Promise<PublicMetrics> {
  return wait(publicMetrics)
}

export async function getDashboardMetrics(): Promise<DashboardMetrics> {
  return wait(buildDashboardMetrics(applicationState))
}
...
export async function getApplicationById(applicationId: string) {
  return wait(applicationState.find((application) => application.id === applicationId) ?? null)
}

export async function getTrendData(): Promise<TrendDataPoint[]> {
  return wait(trendData)
}
...
export async function createApplication(
  ...
  const debtToIncomeRatio =
    ...
    (formData.monthlyIncome ? (((formData.liabilities ?? 0) + totalEmi * 12) / (formData.monthlyIncome * 12)) * 100 : 0)
